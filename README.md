# 👋 Hi there, I'm Sneha Soni

🎓 **B.Tech Student | Web Development Enthusiast**  
🌱 **Currently exploring:** HTML, CSS, JavaScript, Java, DSA  
💻 **Pronouns:** she/her  

---

## 💼 Internship Experience

**Oasis Infobyte (AICTE OIB-SIP Program)** – Web Development & Designing Intern  
🗓 Duration: 1 Month  

- Developed a **Landing Page** using HTML & CSS  
- Built a **Portfolio Website** using HTML & CSS  
- Created a **Temperature Converter** using HTML, CSS & Basic JavaScript  
- Gained hands-on experience in **frontend development** and **JavaScript logic**

---

## 💻 Skills

- **Frontend:** HTML, CSS, Basic JavaScript  
- **Tools:** VS Code, Git, GitHub  
- **Concepts:** Responsive Design, Version Control, Basic JS Logic  
- **Programming Languages:** Java

---

## 📂 Projects

1. **Food Landing Page 🍔** – HTML & CSS  
   - A simple and responsive landing page with beginner-friendly design  
   - **Live Demo:** [Click here](https://SnehaSoni321.github.io/food-landing-page-html-css/)

2. **Landing Page Project 🖥️** – HTML & CSS  
   - Beginner-friendly page focusing on layout and design  
   - **Live Demo:** [Click here](https://SnehaSoni321.github.io/OIBSIP_domain_taskno./Landing_Page/)

3. **Temperature Converter 🌡️** – HTML, CSS & JavaScript  
   - Interactive web app to convert temperatures between Celsius, Fahrenheit, and Kelvin  
   - **Live Demo:** [Click here](https://SnehaSoni321.github.io/OIBSIP_domain_taskno./temperature-converter/)

4. **To-Do List 📝** – HTML, CSS & JavaScript  
   - Allows users to add, delete, and mark tasks as completed  
   - Data stored in browser local storage for persistence  
   - **Live Demo:** [Click here](https://snehasoni321.github.io/To-Do-List/)

---

## 📫 Contact Me

- **LinkedIn:** [Sneha Soni](https://www.linkedin.com/in/sneha-soni-b8288a356)  
- **Email:** snehasoni598@gmail.com
